...
## News
- [2025.09.11] We introduce [LLaDA-MoE-7B-A1B-Base](https://huggingface.co/inclusionAI/LLaDA-MoE-7B-A1B-Base) and [LLaDA-MoE-7B-A1B-Instruct](https://huggingface.co/inclusionAI/LLaDA-MoE-7B-A1B-Instruct) in collaboration with [Ant Group](https://www.antgroup.com/en), the first diffusion language model pretrained from scratch with Mixture of Experts (MoE) architecture. Trained on approximately 20T tokens and instruction-tuned, LLaDA-MoE-7B-A1B-Instruct use only ~1B active parameters at inference while surpassing LLaDA 1.5(an 8B dense model), and comparable to Qwen2.5-3B-Instruct on knowledge, math, coding, reasoning, agent and alignment tasks.
